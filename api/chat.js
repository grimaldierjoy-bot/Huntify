...
PROFIL VOYAGE :
${mergedCtxStr||'voir historique'}

HISTORIQUE :
${hist||'Début'}

════════════════════════════════════════════
RÈGLE BUDGET — ABSOLUMENT CRITIQUE
════════════════════════════════════════════
Le budget donné par le client est LE BUDGET TOTAL TOUT COMPRIS.
AVANT de proposer quoi que ce soit, tu DOIS dispatcher ce budget :

Exemple : budget 1500€ pour 2 personnes, 4 nuits
  → Vols A/R (2 pers)     : ~300€  (estimation réaliste)
  → Hébergement (4 nuits)  : ~600€  (150€/nuit pour 2)
  → Restaurants (4 soirs)  : ~240€  (60€/soir pour 2)
  → Activités/visites       : ~200€
  → Transport local          : ~80€
  → Marge imprévus (5%)     : ~80€
  TOTAL                     : ~1500€ ✅

TU NE PEUX PAS proposer un hôtel à 200€/nuit si le budget hébergement calculé est 150€/nuit.
TU NE PEUX PAS proposer des vols à 500€ si le budget vols calculé est 300€.
CHAQUE proposition doit rentrer dans le budget dispatché.

Si le budget est serré → propose des options réalistes et honnêtes, pas du luxe impossible.
Si le budget est confortable → propose des options de qualité adaptées.
════════════════════════════════════════════
...
2. HÔTELS — Recherche les VRAIS prix sur Booking.com pour les dates exactes du client :
   ...
   → Propose : 3 hôtels nommés (budget/confort/luxe) dans la fourchette de prix réaliste
   → Chaque hôtel : nom exact, prix/nuit RÉEL, quartier, point fort, lien Booking direct
...
{"type":"itinerary","recap":"...","itinerary":{"destination":"Lisbonne","country":"Portugal","flag":"🇵🇹","duration":"7 jours","travelers":"2 adultes","style":"culture","flights":{"outbound":{"from":"Paris CDG","to":"Lisbonne LIS","price":"145€","airline":"TAP Air Portugal","duration":"2h30","link":null},"return":{"from":"Lisbonne LIS","to":"Paris CDG","price":"145€","airline":"TAP Air Portugal","duration":"2h30","link":null}},"hotels":[{"name":"Hotel do Chiado","stars":4,"price":"95","location":"Chiado","highlight":"Vue sur les toits","booking_link":null,"category":"confort","minPrice":80,"maxPrice":120},{"name":"Yes! Lisbon Hostel","stars":3,"price":"40","location":"Mouraria","highlight":"Ambiance locale","booking_link":null,"category":"budget","minPrice":30,"maxPrice":60},{"name":"Bairro Alto Hotel","stars":5,"price":"290","location":"Bairro Alto","highlight":"Vue panoramique","booking_link":null,"category":"luxe","minPrice":200,"maxPrice":400}],"days":[{"num":1,"title":"Arrivée et Alfama","morning":"Arrivée LIS, metro vers le centre (1.65€)","afternoon":"Quartier Alfama, Miradouro da Graça — gratuit","evening":"Découverte du Bairro Alto animé","restaurant":{"name":"Solar dos Presuntos","price":"35€/2 pers","specialty":"Bacalhau traditionnel"},"activities":["Miradouro da Graça — gratuit","Cathédrale Sé — 5€/pers"],"hotel":"Hotel do Chiado","budget":90}],"budget":{"flights_total":290,"accommodation_total":665,"activities_total":150,"food_total":280,"transport_local":40,"total":1425,"per_person":712,"note":"Prix relevés en juin 2026, variables selon dates exactes"},"tips":["Réserver vols 6-8 semaines à l'avance","Metro : 10 trajets = 9.10€","Éviter août — préférer mai/juin ou septembre","Lisboa Card 24h = 20€ : transports + musées gratuits","App MB Way pour payer sans frais"]}}`;
      ...
      const tSysConv = `Tu es un agent voyage expert pour Huntify.
Tu collectes les infos nécessaires pour créer un itinéraire complet avec VRAIS prix.
...
4. Style du voyage (culturel / plage / gastronomie / romantique / aventure)
5. Budget total — demande si vols inclus ou non

Quand tu as destination + dates + ville départ + style → type:ready.

JSON UNIQUEMENT :
{"type":"question","message":"ta question"}
{"type":"ready","recap":"destination, dates, voyageurs, ville_depart, style, budget, vols_inclus"}`;

      // ⚡ Phase 1 voyage : Claude directement
      // Le voyage est trop important pour les IA gratuites
      // Claude comprend mieux le besoin, la psychologie, et ne boucle pas
      let tRaw = await callClaude(tSysConv, tUser, 400);
      ...
      // Phase 2 : Claude TOUJOURS avec web search
      ...
      if(itin.hotels?.length) {
        ...
        for(const h of itin.hotels) {
          ...
          if (h.name && h.name.length > 3 && h.name !== 'null') {
            // Sélection IA : hôtel précis avec avantages + lien direct
            ...
            const descs  = {budget:'Hôtels et hostels bien notés',confort:'Hôtels 3-4 étoiles confortables',luxe:'Hôtels 4-5 étoiles premium'};
            const catLabels = {budget:'💚 Budget',confort:'💙 Confort',luxe:'💎 Luxe'};
            html+=hotelRangeCard(h.category,catLabels[h.category]||'🏨',
              h.price?h.price+'€/nuit':ranges[h.category]||'Voir prix',
              h.highlight||descs[h.category]||'',
              buildBookingLink(itin.destination||'',nights,2,h.minPrice||null,h.maxPrice||null));
          }
        }

        // Lien "Voir d'autres hôtels" — filtre Booking autour des prix proposés
