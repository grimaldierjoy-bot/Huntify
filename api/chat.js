...
function promoBox(code, store, desc) {
  return `<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:1.5px solid #86efac;border-radius:12px;padding:12px 14px;margin-top:8px">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
      <div style="flex:1">
        <div style="font-size:11px;color:#16a34a;font-weight:700;margin-bottom:2px">🏷️ Code promo ${store} ✅</div>
        <div style="font-size:13px;color:#166534;font-weight:600">${desc}</div>
      </div>
      <div onclick="navigator.clipboard.writeText('${code}');this.innerHTML='✓';setTimeout(()=>this.innerHTML='${code}',2000)" style="background:#16a34a;color:#fff;border-radius:8px;padding:8px 12px;font-weight:800;font-size:13px;cursor:pointer;white-space:nowrap">${code}</div>
    </div>
  </div>`;
}
...
function priceHistoryBox(old, trend) {
  ...
  const msg = trend==='down' ? `Prix en baisse ! Était à ${old} — bonne affaire ✅` : trend==='up' ? `⚠️ Prix gonflé ! Était à ${old} avant` : `Prix stable depuis 30 jours`;
...
export default async function handler(req) {
  ...
  try {
    ...
    // ══════════════════════════════════════════
    // APPEL 1 — RECHERCHE WEB LIBRE (l'agent explore)
    // ══════════════════════════════════════════
    const searchResp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: `Tu es l'agent shopping de Huntify. Boutiques : ${activeNames}.
Recherche sur le web et trouve, pour le produit demandé :
1. Les prix réels actuels sur Amazon.fr et Rakuten
2. Les codes promo actifs (cherche sur dealabs.com, ma-reduc.com)
Présente tes trouvailles en texte libre détaillé : noms exacts, prix constatés, codes promos avec leur réduction. Sois précis sur les chiffres.`,
        messages: [{ role:'user', content:`Recherche prix et codes promo pour : ${message}` }]
      })
    });
    ...
    // ══════════════════════════════════════════
    // APPEL 2 — STRUCTURATION JSON (sans web search)
    // ══════════════════════════════════════════
    ...
    const structResp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: 'claude-haiku-4-5',
        max_tokens: 1500,
        system: `Tu structures des résultats de recherche shopping en JSON.
Boutiques valides (champ store) : ${slugs}.
Retourne UNIQUEMENT ce JSON, rien d'autre :
{
  "summary": "phrase de conseil avec le meilleur prix trouvé",
  "products": [
    {"name":"nom exact du produit","price":"prix avec €","store":"amazon","keywords":"mots clés courts"},
    {"name":"nom exact","price":"prix avec €","store":"rakuten","keywords":"mots clés courts"}
  ],
  "promoCodes": [
    {"code":"CODE","store":"nom boutique","discount":"ex: -10% ou 5€ offerts"}
  ]
}
Extrais les VRAIS prix du texte. Si pas de prix précis pour une boutique, mets le prix le plus probable mentionné. promoCodes:[] si aucun code dans le texte.`,
        messages: [{ role:'user', content:`Structure ces résultats de recherche en JSON :\n\n${searchText}` }]
      })
    });
    ...
    if (!products.length) {
      products = advertisers.slice(0,2).map(a=>({name:message,price:'Voir prix',store:a.slug,keywords:message}));
    ...
    if (mainProduct?.price && mainProduct.price !== 'Voir prix') {
      ...
      if (!isNaN(cur)) sbFetch('price_history','POST',{product_id:slug,product_name:mainProduct.name,price:cur,store:'amazon',url:null});
    }
    ...
    for (const p of products) {
      ...
      if (!adv) continue;
      const url = buildAffiliateLink(adv, p.keywords||p.name);
      if (!url) continue;
