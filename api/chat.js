...
export default async function handler(req) {
  ...
  try {
    ...
    // Règle d'or produit : si on comprend ce que l'utilisateur veut → on cherche IMMÉDIATEMENT
    // Le budget est optionnel. Un nom de produit suffit.
    ...
    // On cherche si : produit compris OU budget donné OU déjà 1 échange OU 1 question posée
    ...
    if (!mustSearch) {
      const p1sys = 'Tu es l assistant shopping Huntify. REGLE D OR: si tu comprends le produit recherche → ready:true IMMEDIATEMENT.\n'
        + 'Un nom de produit seul suffit (mascara, casque sony, airfryer...). Le budget est optionnel.\n'
        + 'Pose une question UNIQUEMENT si la demande est vraiment incomprehensible (ex: "un truc").\n'
        + 'JAMAIS de question sur: type exact, couleur, caracteristiques techniques, marque precise.\n'
        + 'HISTORIQUE: ' + (hist||'Debut') + '\n'
        + 'Si ' + qAsked + ' >= 1 → ready:true OBLIGATOIRE, on cherche avec ce qu on a.\n'
        + 'Recap = produit + budget si mentionne. JSON: {ready:true,recap:mots-cles} ou {ready:false,msg:question tres courte}';
    ...
    // Recap enrichi : combine message + historique pour contexte complet
    const allUserMsgs = (history||[]).filter(m=>m.role==='user').map(m=>m.content||'').join(' ');
    const recap  = decision.recap || (allUserMsgs ? allUserMsgs + ' ' + message : message).trim();
    ...
    if (effective === 'groq_search') {
      const groqPrompt = 'Agent shopping Huntify. Recherche MAINTENANT sur amazon.fr et fr.shopping.rakuten.com.\n'
        ...
        + '2. Le nom du produit DOIT être le vrai nom complet (marque + modèle exact) tel qu il apparait sur le site\n'
        + '   JAMAIS de noms generiques comme "Masque de snorkeling" ou "Casque audio" seuls\n'
        + '   TOUJOURS: "Decathlon Easybreath 500", "Sony WH-1000XM5", "Philips Airfryer HD9252"\n'
    ...
    // ── Claude + web_search (premium)
      ...
      const p2sys = 'Agent shopping Huntify. Cherche sur Amazon.fr et Rakuten.fr.\n'
        ...
        + 'Exemples INTERDITS: "Casque audio", "Masque de plongée", "Airfryer pas cher"\n'
    ...
    // ── Free fast (Groq sans search)
      ...
      const p2sys = 'Agent shopping Huntify. Boutiques: ' + activeNames + '.\n'
        ...
        + '2 produits Amazon + 1 Rakuten. JSON: {summary, products:[{name,price,store,keywords,url,badge}], promoCodes:[]}';      const raw = await callFreeAI(p2sys, `BESOIN: ${recap}`, 'fast');
      const p   = parseJSON(raw || '');
      products  = p.products  || [];
      promoCodes= p.promoCodes|| [];
      summary   = p.summary   || '';
