...
//
// PIPELINE :
//   1. route()    → IA gratuite décide question vs génération (plafond 2 Q)
//   2. generate()  → IA gratuite (Groq compound-beta, recherche web) construit
//                    l'itinéraire complet (hôtels réels, prix réalistes)
//   3. finalize()  → Claude + web_search, UNIQUEMENT si (2) échoue. Plafonné.
//
// ENRICHISSEMENTS v4 :
//   - Météo réelle (Open-Meteo, gratuit, sans clé) sur les dates du voyage
//   - Budget ventilé par jour (déjà dans days[].budget) + budget global détaillé
//   - Transport local : champ dédié dans le JSON itinéraire (rempli par l'IA)
//   - Multi-turn renforcé : les infos s'accumulent sur toute la conversation
// ─────────────────────────────────────────────────────────────────────────────

import { sb, buildHist, countQ, getWeather, MAX_QUESTIONS, AWIN_PUB } from './_lib/shared.js';
...
// Recherche Booking sur le NOM EXACT de l'hôtel proposé par l'IA → le lien
// correspond toujours à la carte affichée (garde-fou anti-décalage)
function bookHotelLink(hotelName, dest, ci, co, adults) {
  const q = (hotelName || "") + " " + (dest || "");
  let url = "https://www.booking.com/searchresults.html?ss=" + encodeURIComponent(q.trim())
...
function parseDate(str) {
  ...
  if (fm) {
    let mo = null; for (const mk in MO) if (fm[2].startsWith(mk)) { mo = MO[mk]; break; }
    if (mo) { const yr = fm[3] || String(now.getFullYear()); return yr + "-" + String(mo).padStart(2, "0") + "-" + fm[1].padStart(2, "0"); }
  }
  return null;
}

// Itinéraire exploitable = destination + au moins un hôtel au vrai nom
...
function cardWeather(w, label) {
  ...
  return '<div style="display:flex;align-items:center;gap:8px;background:#eff6ff;border-radius:10px;padding:8px 12px;margin-top:6px">'
    ...
    + '<div style="font-size:11.5px;color:#1e40af;font-weight:600">' + label + ' : ' + w.tempMin + '° / ' + w.tempMax + '°C'
    + (w.rainPct != null ? ' · ' + w.rainPct + '% pluie' : '') + '</div></div>';
}
...
function cardLocalTransport(t) {
  ...
  return '<div style="background:#fffbeb;border:1.5px solid #fde68a;border-radius:14px;padding:12px 14px;margin-top:10px">'
    ...
    + '<div style="font-size:12px;color:#78350f;line-height:1.5">' + (t.desc || '') + (t.price ? ' <b>· ~' + t.price + '</b>' : '') + '</div>'
    + '</div>';
}
...
export default async function handler(req) {
  ...
  try {
    ...
    // ── ÉTAPE 1 : DÉCISION multi-turn (orchestrator.route — IA gratuite) ─────
    // L'IA lit TOUT l'historique pour accumuler les infos au fil des tours.
    // Le code impose seulement le plafond MAX_QUESTIONS (anti-boucle) — la
    // question elle-même n'est jamais écrite en dur, c'est l'IA qui la formule.
    ...
    const { parsed: step1 } = await route(decidePrompt, message, 700);
    ...
    // ── ÉTAPE 2 : GÉNÉRATION (orchestrator.generate — Groq compound-beta) ────
    ...
    let { parsed: tP } = await generate(genPrompt, 2500);
    let itin = tP.itin;
    ...
    if (!usableItin(itin)) {
      const { parsed: cP } = await finalize(
        ...
        + 'N invente jamais un nom d hotel.',
        genPrompt, 1800, true
      );
      if (usableItin(cP.itin)) { itin = cP.itin; tP = cP; }
    }

    // Fallback minimal ultime — liens directs, toujours fonctionnels
    if (!itin) {
    ...
    // Transferts + transport local + location voiture
    if (finalCi) {
    ...
    // Contexte structuré renvoyé au frontend pour alimenter la sidebar live
